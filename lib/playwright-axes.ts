...
export async function analyzeAccessibility(
  ...
  try {
    ...
    const summary = {
      ...
      serious: allResults.filter((r) => r.severity === "High").length,
      moderate: allResults.filter((r) => r.severity === "Medium").length,
      minor: allResults.filter((r) => r.severity === "Low").length,
...
function processAxeResults(results: any, url: string): AccessibilityResult[] {
  ...
  return violations.flatMap((violation: any) => {
    return violation.nodes.map((node: any, index: number) => {
      // Map axe impact to severity (matching Prisma enum)
